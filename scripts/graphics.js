game.Graphics = function (id) {
  ...
  function renderStats() {
    ...
    for (var i = 0; i < game.lives; i++) {
      context.drawImage(
        game.images["images/livesDisplay.png"],
        startX + i * 50,
        1025,
        42,
        49,
  ...
  function Texture(spec) {
    ...
    that.update = function (elapsedTime) {
      if (spec.lifetime != null) {
        //check if this is an object with a lifetime
        spec.lifetime -= elapsedTime; //decrement it's lifetime
        if (spec.lifetime < 0) that.toBeDeleted = true;
      }
      spec.center.x +=
        that.velocityVector.x * (that.thrustPerSecond * (elapsedTime / 1000));
      spec.center.y +=
        that.velocityVector.y * (that.thrustPerSecond * (elapsedTime / 1000));
      that.x = spec.center.x;
      that.y = spec.center.y;
    ...
    that.moveUp = function (elapsedTime) {
      that.velocityVector.x +=
        that.directionVector.x * (that.thrustPerSecond * (elapsedTime / 1000));
      that.velocityVector.y +=
        that.directionVector.y * (that.thrustPerSecond * (elapsedTime / 1000));

      if (id == "asteroids") maxSpeed = 40;
      else maxSpeed = 20;

      that.velocityVector.x =
        that.velocityVector.x > maxSpeed ? maxSpeed : that.velocityVector.x;
      that.velocityVector.y =
        that.velocityVector.y > maxSpeed ? maxSpeed : that.velocityVector.y;
      that.velocityVector.x =
        that.velocityVector.x < -maxSpeed ? -maxSpeed : that.velocityVector.x;
      that.velocityVector.y =
        that.velocityVector.y < -maxSpeed ? -maxSpeed : that.velocityVector.y;
