...
game.collided = function (a, b) {
  if (
    Math.sqrt((b.x - a.x) * (b.x - a.x) + (b.y - a.y) * (b.y - a.y)) <= a.radius
  ) {
    return true;
  }
...
game.wrapAroundDistance = function (x0, y0, x1, y1) {
  //returns the shortest distance between two points,  accounts for wrap around at edges.
  ...
  var two = Math.sqrt(
    (x0 - shiftedX) * (x0 - shiftedX) + (y0 - y1) * (y0 - y1),
  ); //shifted x
  var three = Math.sqrt(
    (x0 - x1) * (x0 - x1) + (y0 - shiftedY) * (y0 - shiftedY),
  ); //shifted y
  var four = Math.sqrt(
    (x0 - shiftedX) * (x0 - shiftedX) + (y0 - shiftedY) * (y0 - shiftedY),
  ); //shifted x and y

  return Math.min(one, two, three, four);
};

game.timeDelay = function (milliseconds) {
  //A time delay that can be used to slow down the program to debug it.
...
game.generateAnAsteroid = function (
  asteroidClass,
  coordinates,
  canvas,
  asteroidsInPlay,
