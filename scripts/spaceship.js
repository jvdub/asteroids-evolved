game.spaceship = function () {
  ...
  function generateParticles() {
    for (var i = 0; i < 5; ++i) {
      particles[1].create(
        spaceship.x -
          spaceship.directionVector.x * 71 +
          18 * -spaceship.directionVector.y,
        spaceship.y -
          spaceship.directionVector.y * 71 +
          18 * spaceship.directionVector.x,
        Math.PI +
          Math.atan2(spaceship.directionVector.y, spaceship.directionVector.x),
      );
      particles[2].create(
        spaceship.x -
          spaceship.directionVector.x * 71 -
  ...
  function fireMissile(bulletsInPlay) {
    //incorporates speed of the shipinto speed of the projectile
    // Prevent a missile from firing if one has just been fired.
    if (game.bulletIntervalCountdown < 0 && !coordinates.toBeDeleted) {
      var directionMagnitude = Math.sqrt(
        Math.pow(spaceship.velocityVector.x, 2) +
          Math.pow(spaceship.velocityVector.y, 2),
      );
      ...
      var angleBetweenVectors = Math.acos(
        spaceship.directionVector.x * directionUnitX +
          spaceship.directionVector.y * directionUnitY,
      );
      var momentumIncreaseFactor =
        (Math.PI / 2 - angleBetweenVectors) / (Math.PI / 2);
      ...
      bulletsInPlay.push(
        graphics.Texture({
          ...
          center: {
            x: spaceship.x + spaceship.directionVector.x * 50,
            y: spaceship.y + spaceship.directionVector.y * 50,
          },
          width: 20,
          height: 20,
          rotation: 100,
          ...
          startVector: {
            x: spaceship.directionVector.x,
            y: spaceship.directionVector.y,
          },
  ...
  function init(ship, isAttractMode) {
    graphics =
      isAttractMode === true
        ? game.Graphics("attract-asteroids")
        : game.Graphics("asteroids");
      ...
      particleSystem(
        {
          ...
          center: {
            x: spaceship.x - spaceship.directionVector.x * 79,
            y: spaceship.y,
          },
          speed: { mean: 0.5, stdev: 0.25 },
          lifetime: { mean: 100, stdev: 50 },
          direction: Random.nextGaussian(
      ...
      particleSystem(
        {
          ...
          center: {
            x: spaceship.x - spaceship.directionVector.x * 71,
            y: spaceship.y - spaceship.directionVector.y * 18,
          },
          speed: { mean: 0.5, stdev: 0.25 },
  ...
  function getShipAngleToTarget(target) {
    return Math.acos(
      spaceship.directionVector.x * target.x +
        spaceship.directionVector.y * target.y,
    );
  }
